...
watch(
  () => store.geminiApiKey,
  (newKey) => {
    if (newKey && !geminiKeyInput.value) {
      geminiKeyInput.value = newKey
    }
  },
  { immediate: true },
)
...
function handleSaveGeminiKey() {
  store.setGeminiApiKey(geminiKeyInput.value)
  isGeminiSaved.value = true
  setTimeout(() => {
    isGeminiSaved.value = false
  }, 2000)
}
  ...
  <div class="max-w-[714px] mx-auto space-y-6 animate-in fade-in-50 slide-in-from-bottom-3 duration-300">
    ...
    <div class="rounded-xl border border-border bg-card p-6 space-y-4 shadow-xs">
      <div class="space-y-1">
        ...
        <p class="text-xs text-muted-foreground leading-relaxed">
          Google AI Studio API key for Google Gemini 3.5 Transcribe smart speech recognition. Free tier supported directly from your browser.
        </p>
      </div>
      ...
      <div class="space-y-3">
        ...
        <div class="flex items-center justify-between pt-1">
          ...
          >
            <Check v-if="isGeminiSaved" class="w-3.5 h-3.5 text-emerald-400" />
            <span>{{ isGeminiSaved ? 'Saved' : 'Save Gemini Key' }}</span>
          </Button>
        </div>
      </div>
    </div>
